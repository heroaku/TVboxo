        ...
        for (let [cateId, cateName] of Object.entries(classMap)) {
            ...
            if (cateId === 'dianying' || cateId === 'jilupian') {
                ...
                for (let a of areaLinks) {
                    let href = pdfh(a, 'a&&href');
                    let match = href.match(/area\\/([^\\/]+)/);
                    if (match) {
                        let area = decodeURIComponent(match[1]);
                        if (!areaValues.find(x => x.n === area)) {
                ...
                // 电视剧/综艺/动漫/短剧：提取 class
                let classValues = [{ n: "全部", v: "" }];
                let classLinks = pdfa(html, 'a[href*="/class/"]');
                for (let a of classLinks) {
                    let href = pdfh(a, 'a&&href');
                    let match = href.match(/class\\/([^\\/]+)/);
                    if (match) {
                        let cls = decodeURIComponent(match[1]);
                        if (!classValues.find(x => x.n === cls)) {
            ...
            for (let a of yearLinks) {
                let href = pdfh(a, 'a&&href');
                let match = href.match(/year\\/([^\\/]+)/);
                if (match && /^\d{4}$/.test(match[1])) {
                    let year = match[1];
                    if (!yearValues.find(x => x.n === year)) {
