var rule = {
    ...
    一级: $js.toString(() => {
        ...
        let fyclass = input.split('/show/')[1].split('/')[0];
        let fl = {};
        try { fl = JSON.parse(input.split('fl=')[1]?.split('&')[0] || '{}'); } catch(e) {}

        let parts = [fyclass];
        if (fl.cateId && fl.cateId !== fyclass) parts[0] = fl.cateId;

        if (fyclass === 'dianying' || fyclass === 'jilupian') {
            if (fl.area && fl.area !== "") parts.push('area', fl.area);
        } else {
            if (fl.class && fl.class !== "") parts.push('class', fl.class);
        }

        parts.push('by', fl.by || 'year');
        if (fl.year && fl.year !== "") parts.push('year', fl.year);
    ...
    二级: $js.toString(() => {
        ...
        let vod_content = pdfh(html, '#blurb_' + vod_name.replace(/[\s"'\uFF01-\uFFFF]+/g, '_') + '&&Text') || '';
